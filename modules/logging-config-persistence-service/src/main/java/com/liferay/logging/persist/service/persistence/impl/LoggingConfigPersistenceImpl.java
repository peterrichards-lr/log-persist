...
import com.liferay.logging.persist.service.persistence.LoggingConfigUtil;
 ...
 * @generated
 */
@Component(service = {LoggingConfigPersistence.class, BasePersistence.class})
public class LoggingConfigPersistenceImpl
	...
	public LoggingConfig fetchByLogger(String logger, boolean useFinderCache) {
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByLogger, finderArgs);
		}
	...
	public LoggingConfigPersistenceImpl() {
		...
		setTable(LoggingConfigTable.INSTANCE);
	}
	...
	public void cacheResult(List<LoggingConfig> loggingConfigs) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (loggingConfigs.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public void clearCache() {
		...
		finderCache.clearCache(LoggingConfigImpl.class);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		finderCache.putResult(_finderPathCountByLogger, args, Long.valueOf(1));
		finderCache.putResult(
			_finderPathFetchByLogger, args, loggingConfigModelImpl);
	}
	...
	public void activate() {
		...
		_finderPathFetchByLogger = new FinderPath(
			FINDER_CLASS_NAME_ENTITY, "fetchByLogger",
			new String[] {String.class.getName()}, new String[] {"logger"},
			true);

		_finderPathCountByLogger = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByLogger",
			new String[] {String.class.getName()}, new String[] {"logger"},
			false);

		_setLoggingConfigUtilPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		_setLoggingConfigUtilPersistence(null);

		entityCache.removeCache(LoggingConfigImpl.class.getName());
	}

	private void _setLoggingConfigUtilPersistence(
		LoggingConfigPersistence loggingConfigPersistence) {

		try {
			Field field = LoggingConfigUtil.class.getDeclaredField(
				"_persistence");

			field.setAccessible(true);

			field.set(null, loggingConfigPersistence);
		}
	...
	@Reference
	private LoggingConfigModelArgumentsResolver
		_loggingConfigModelArgumentsResolver;
